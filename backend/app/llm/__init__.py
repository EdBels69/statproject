...
def _normalize_usage(
    ...
    *,
    ...
    base = usage.copy() if isinstance(usage, dict) else {}
    prompt_tokens = _safe_int(base.get("prompt_tokens"))
    completion_tokens = _safe_int(base.get("completion_tokens"))
    total_tokens = _safe_int(base.get("total_tokens"))
...
def _finalize_usage(
    ...
    *,
    ...
    out: Dict[str, Any] = usage.copy() if isinstance(usage, dict) else {
        "prompt_tokens": 0,
        "completion_tokens": 0,
        "total_tokens": 0,
    }
    out["prompt_tokens"] = _safe_int(out.get("prompt_tokens"))
    out["completion_tokens"] = _safe_int(out.get("completion_tokens"))
    out["total_tokens"] = _safe_int(out.get("total_tokens"))
    out["model_requested"] = str(requested_model or "")
    out["model_used"] = str(used_model or "")
    out["fallback_model"] = str(fallback_model or "")
    out["fallback_used"] = bool(used_model and str(used_model) != str(requested_model))
    out["attempt_count"] = int(len(attempts))
    out["attempts"] = attempts
    return out


def _should_auto_failover_to_gemini(model: str) -> bool:
    ...
    if "gemini" in model_norm:
        return False
    return (
        model_norm.startswith("minimax/")
        or model_norm.startswith("z-ai/")
        or model_norm.startswith("qwen/")
        or model_norm.startswith("glm")
    )


def _resolve_api_failover_model(requested_model: str) -> Optional[str]:
    if not bool(getattr(settings, "CLINIMETRIA_LLM_FAILOVER_ENABLED", True)):
        return None
    if not _should_auto_failover_to_gemini(requested_model):
        return None
    configured = str(getattr(settings, "CLINIMETRIA_MODEL_API_FALLBACK", "") or "").strip()
    if configured and configured.lower() != str(requested_model or "").strip().lower():
        return configured
    default_model = "google/gemini-2.5-flash"
    if default_model.lower() == str(requested_model or "").strip().lower():
        return None
...
async def _chat_completion(
    *,
    ...
    async def _run_model(target_model: str) -> Tuple[Optional[str], Dict[str, Any], List[Dict[str, Any]]]:
        attempts: List[Dict[str, Any]] = []
        url, api_key = _resolve_llm_target(target_model)
        ...
        cache_key = _make_llm_cache_key(model=target_model, url=str(url), payload=payload)
        ...
        if cached:
            ...
            return cached, usage_cached, attempts
        ...
        async with httpx.AsyncClient(timeout=timeout_s) as client:
            for attempt in range(1, retry_attempts + 1):
                ...
                try:
                    ...
                    if content:
                        usage = _normalize_usage(data.get("usage"), prompt=prompt, content=content)
                        attempts.append(
                            _build_attempt_row(
                                model=target_model,
                                url=url,
    ...
    content, usage_primary, attempts_primary = await _run_model(requested_model)
    all_attempts.extend(attempts_primary)
    if content:
        return content, _finalize_usage(
            usage_primary,
    ...
    fallback_model = _resolve_api_failover_model(requested_model)
    primary_failed_with_timeout = False
    if attempts_primary:
        last_error = attempts_primary[-1] if isinstance(attempts_primary[-1], dict) else {}
        err_text = str(last_error.get("error") or "").lower()
        primary_failed_with_timeout = "timeout" in err_text

    if fallback_model and (primary_failed_with_timeout or attempts_primary):
        content_fallback, usage_fallback, attempts_fallback = await _run_model(fallback_model)
        all_attempts.extend(attempts_fallback)
        if content_fallback:
            usage_done = _finalize_usage(
                usage_fallback,
                requested_model=requested_model,
            ...
            usage_done["fallback_reason"] = "timeout_or_provider_error"
            return content_fallback, usage_done

    return None, _finalize_usage(
        {},
        requested_model=requested_model,
        used_model=None,
        fallback_model=fallback_model,
...
def _extract_json_object(text: str) -> str:
    ...
    def _extract_balanced_fragment(opening: str, closing: str) -> Optional[str]:
    ...
    object_pos = source.find("{")
    array_pos = source.find("[")
    candidates: List[Tuple[int, str, str]] = []
    if object_pos != -1:
        candidates.append((object_pos, "{", "}"))
    if array_pos != -1:
        candidates.append((array_pos, "[", "]"))
    if not candidates:
        return source

    for _, opening, closing in sorted(candidates, key=lambda item: item[0]):
        fragment = _extract_balanced_fragment(opening, closing)
        if fragment:
            return fragment
    return source
...
def _repair_json_syntax(candidate: str) -> str:
    ...
    text = re.sub(r'(?<=[\]}\d"])(\s+)(?=(?:true|false|null)\b)', r",\1", text)
...
- config:
  ...
  - Для корреляций: outcome и group обязательны. Можно дополнительно указать correlation_method (pearson|spearman|kendall).
  ...
  - Для любых числовых сравнений можно задавать normality_test (shapiro|dagostino|anderson|ks|suite|auto), normality_decision (majority|all|any), homogeneity_test (levene|bartlett|fligner), homogeneity_center (median|mean|trimmed).
...
"""
    ...
    logger.warning("LLM Research Design parse failed; entering repair path.")
...
"""
        try:
            ...
            if fixed_content:
                parsed = _parse_json_response(fixed_content)
                if isinstance(parsed, dict):
                    if usage_log and isinstance(preferences, dict) and preferences.get("return_usage"):
                        parsed["usage"] = usage_log
                    return parsed
    ...
    try:
        ...
        if retry_content:
            parsed = _parse_json_response(retry_content)
            if isinstance(parsed, dict):
                if usage_log and isinstance(preferences, dict) and preferences.get("return_usage"):
                    parsed["usage"] = usage_log
                return parsed
...
"""
            ...
            if fallback_content:
                parsed = _parse_json_response(fallback_content)
                if isinstance(parsed, dict):
                    if usage_log and isinstance(preferences, dict) and preferences.get("return_usage"):
                        parsed["usage"] = usage_log
                    return parsed
