...
class SorcererRecommendRequest(BaseModel):
    goal: Literal[
        "compare_groups",
        "compare_timepoints",
        "relationship",
...
async def recommend(payload: SorcererRecommendRequest):
    goal = payload.goal
...
class SorcererApplyRequest(BaseModel):
    dataset_id: str
...
async def apply(payload: SorcererApplyRequest):
    ...
    try:
        if method_id in {"rm_anova", "friedman"}:
            ...
            return _sanitize_json({"results": out})
