...
def _analysis_set_parquet_path(base_dir: str, dataset_id: str, analysis_set_id: str) -> str:
    safe_id = str(analysis_set_id or "").strip()
    return os.path.join(_analysis_sets_dir(base_dir, dataset_id), f"{safe_id}.parquet")


def _analysis_set_hash_path(base_dir: str, dataset_id: str) -> str:
    return os.path.join(str(base_dir), str(dataset_id), "processed", "analysis_set_hash.json")
...
def _write_analysis_set_hash(base_dir: str, dataset_id: str, artifact: Dict[str, Any]) -> None:
    try:
        ...
        digest = None
    payload = {
        "schema": "clinimetria.analysis_set_hash",
        "version": 1,
        "dataset_id": str(dataset_id),
        "analysis_set_id": artifact.get("analysis_set_id") if isinstance(artifact, dict) else None,
        "analysis_set_sha256": digest,
        "fingerprint": artifact.get("fingerprint") if isinstance(artifact, dict) else None,
        "updated_at": _utc_iso(),
    }
    _write_json_atomic(base_dir, _analysis_set_hash_path(base_dir, dataset_id), payload)
...
def clear_current_analysis_set(base_dir: str, dataset_id: str) -> bool:
    pointer_path = _current_pointer_path(base_dir, dataset_id)
    hash_path = _analysis_set_hash_path(base_dir, dataset_id)
    try:
...
def freeze_analysis_set(
    ...
    *,
    ...
    selected_df = df.loc[indices].copy()
    if mode_norm == "simple_impute" and isinstance(imputation, dict):
        values = imputation.get("values") if isinstance(imputation.get("values"), dict) else {}
        for col, fill_val in values.items():
            if col not in selected_df.columns:
                continue
            try:
                col_mask = _series_nonmissing_mask(selected_df[col])
                selected_df.loc[~col_mask, col] = fill_val
            except Exception:
                pass

    parquet_path = _analysis_set_parquet_path(base_dir, dataset_id, analysis_set_id)
    selected_df.to_parquet(parquet_path, index=False)
    parquet_rel = os.path.join("processed", "analysis_sets", f"{analysis_set_id}.parquet")
    artifact["analysis_set_parquet"] = parquet_rel
    artifact["analysis_set_sha256"] = _sha256_file(parquet_path)
    ...
    _write_analysis_set_hash(base_dir, dataset_id, artifact)
    return artifact
