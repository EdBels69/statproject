...
class CopilotEngine:
    ...
    def __init__(self):
        ...
        self.model = os.getenv("LLM_MODEL_ID", "google/gemini-2.5-flash")
    ...
    async def _llm_call(
        ...
        # Fallback to Gemini Flash if primary failed
        if model != "google/gemini-2.5-flash":
            print(f"⚠️ Primary model {model} failed. Switching to fallback: google/gemini-2.5-flash")
            try:
                response, usage = await _chat_completion(
