...
from .nlq_router import nlq_to_protocol
...
@router.post("/nlq")
async def nlq_analyze(req: NLQRequest):
    """
    🧠 Natural Language Query → Protocol.
    Parses the query, selects a domain template, builds a protocol.
    Falls back to LLM planning when no template matches.
    """
    dataset_info = get_dataset_info(req.dataset_id)
    columns = dataset_info.get("columns", [])

    result = nlq_to_protocol(req.query, columns, alpha=req.alpha)
...
async def list_domain_templates():
    """📋 List available domain templates for protocol generation."""
    from app.modules.domain_templates import list_templates
