...
def test_render_plot_line_series_does_not_use_roc_diagonal(monkeypatch):
    colors_seen = []
    original_plot = reporting.plt.plot
    ...
    payload = {
        "type": "time_series",
        "plot_data": [
            {"x": "2026-01-01", "y": 1.2, "trend": 1.1},
            {"x": "2026-01-02", "y": 1.3, "trend": 1.2},
            {"x": "2026-01-03", "y": 1.5, "trend": 1.3},
        ],
        "forecast": {"points": [{"x": "2026-01-04", "y": 1.6}]},
    ...
    assert png.startswith(b"\x89PNG")
    assert "#123456" in colors_seen
    assert "#654321" not in colors_seen


def test_render_plot_bland_altman_draws_reference_lines(monkeypatch):
    line_colors = []
    original_axhline = reporting.plt.axhline

    def _capture_axhline(*args, **kwargs):
        color = kwargs.get("color")
        if isinstance(color, str):
            line_colors.append(color)
        return original_axhline(*args, **kwargs)

    monkeypatch.setattr(reporting.plt, "axhline", _capture_axhline)
    monkeypatch.setattr(
        reporting,
        "_report_plot_theme",
        lambda: {
    ...
    payload = {
        ...
        "plot_config": {"type": "bland_altman"},
        "plot_reference_lines": {
            "mean_difference": {"y": -0.1},
            "loa_lower": {"y": -1.2},
            "loa_upper": {"y": 1.0},
            "zero": {"y": 0.0},
        },
