...
from app.modules.analysis_set import apply_analysis_set_to_df, freeze_analysis_set
...
def test_apply_analysis_set_rejects_out_of_range_indices():
    df = pd.DataFrame({"y": [1, 0, 1], "x": [1.0, 2.0, 3.0]})
    artifact = {
        "analysis_set_id": "bad_range",
        "mode": "complete_case",
        "row_indices": [0, 1, 3],
    }

    try:
        apply_analysis_set_to_df(df, artifact)
        assert False, "Expected ValueError for out-of-range indices"
    except ValueError as e:
        assert "out of range" in str(e).lower()


def test_simple_impute_normalizes_string_missing_tokens(tmp_path):
    dataset_id = "analysis_set_simple_impute_strings"
    df = pd.DataFrame(
        {
            "y": [1, 0, 1, 0],
            "x1": [1.0, "na", "", None],
    ...
    subset, info = apply_analysis_set_to_df(df, artifact)

    assert len(subset) == 4
    assert int(info.get("normalized_string_missing") or 0) >= 2
    assert int(info.get("imputed_cells") or 0) >= 3
    assert subset["x1"].isna().sum() == 0
