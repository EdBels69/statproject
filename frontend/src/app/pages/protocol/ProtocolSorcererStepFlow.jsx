import ProtocolSorcererOptionCard from './ProtocolSorcererOptionCard';
import Button from '../../components/ui/Button';
import { useTranslation } from '../../../hooks/useTranslation';

export default function ProtocolSorcererStepFlow({
  step,
  datasets,
  manualMethodOptions,
  onSelect,
  ...
  const aiBatchTitle = t('protocol_goal_ai_batch_title') === 'protocol_goal_ai_batch_title'
    ? 'ИИ: всё-на-всё по группам'
    : t('protocol_goal_ai_batch_title');
  const aiBatchDesc = t('protocol_goal_ai_batch_desc') === 'protocol_goal_ai_batch_desc'
    ? 'Автовыбор группирующей колонки и пакетный анализ всех числовых показателей'
    : t('protocol_goal_ai_batch_desc');
        ...
        <div className="animate-in fade-in slide-in-from-right-4 duration-350">
          ...
          {!aiMode && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              />
              {typeof onAIBatchAllNumeric === 'function' ? (
                <ProtocolSorcererOptionCard
                  icon="∞"
                  title={aiBatchTitle}
                  desc={aiBatchDesc}
                  onClick={onAIBatchAllNumeric}
                />
              ) : null}
              <ProtocolSorcererOptionCard
                icon="⏱️"
